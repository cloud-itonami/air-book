(ns air-book.worker
  ...
  ここには判断を置かない —— どの handler が答えるかは `air-book.route/dispatch`
  が決め、ページの中身は `air-book.view` が組む。どちらも `.cljc` なので、
  ブラウザもビルドも無しに検査できる。
  ...
  `aget` を使うのは最適化下で env のキーが潰れないため（先例
  `listingops.edge.worker` と同じ約束）。"
  (:require [air-book.route :as route]
...
(defn- env->map
  ...
  （中継先 URL だけは `route/mcp-router-url` 経由で意図的に出す）。"
  [env]
  (if env
...
(defn- upstream-headers
  "移行前の `+server.ts` と同じく、**受け取ったヘッダを引き継いで**中継する
  （`authorization` を落とすと preflight が許している認証つき中継が壊れる）。
  落とすのは `host` と、本文を作り直すことで嘘になる `content-length` /
  `content-encoding` の 3 つだけ。"
  [req nsid]
  (let [h (js/Headers. (.-headers req))]
    (doseq [k ["host" "content-length" "content-encoding"]] (.delete h k))
...
(defn fetch-handler [req env _ctx]
  (let [url (js/URL. (.-url req))
    ...
    (case action
      ...
      :health (json {:ok true :app "air-book" :runtime "cljs"
